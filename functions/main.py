...
def get_connection(node1, node1_lat, node1_lon, node2, node2_lat, node2_lon):
    if node1 != node2:
        ...
        connections = (
            firestore_client.collection("node_tiles")
            .document(tile)
            .collection("nodes")
            .document(node1)
            .get()
...
def update_feedback(req):
    ...
    feedback_data = doc_ref.get().to_dict()["connections"][f"{node2}"]["clusters"][
        f"{label}"
    ]["feedback"]
    attributes_data = doc_ref.get().to_dict()["connections"][f"{node2}"]["clusters"][
        f"{label}"
    ]["attributes"]
    ...
    firestore_client.collection("node_tiles").document(tile2).collection(
        "nodes"
    ).document(node2).update(
        {f"connections.{node1}.clusters.{label}.feedback": feedback_data}
    )
    if new_attributes != attributes_data:
        doc_ref.update(
            {f"connections.{node2}.clusters.{label}.attributes": new_attributes}
        )
        firestore_client.collection("node_tiles").document(tile2).collection(
            "nodes"
        ).document(node2).update(
            {f"connections.{node1}.clusters.{label}.attributes": new_attributes}
        )
...
def get_rankings(req):
    ...
    all_users = (
        ...
        .order_by("totalDistance", direction=firestore.Query.DESCENDING)
        .stream()
    )
    ranking = -1
    top_10_users = []
    for i, user in enumerate(all_users):
        if i < 10:
            ...
            if "fullname" not in user_dict:
                firestore_client.collection("users").document(user.id).update(
                    {"fullname": "Unknown"}
                )
...
def cluster_users(event: scheduler_fn.ScheduledEvent):
    ...
    docs = users_ref.select(
        ["attributes"]
    ).stream()  # docs는 제너레이터 객체. 좀 큰 데이터에서 받아올 때는 stream으로
    docs1 = firestore_client.collection(
        "clusters"
    ).get()  # docs1은 리스트 객체. 작은 데이터라 get으로

    Users = [User(doc.id, doc.to_dict()["attributes"]) for doc in docs]
    Datas = DataFrame.from_dict(
        {user.uid: user.attributes for user in Users}, orient="index"
    )
    num_users = len(Users)
    ...
    if num_users == 1:
        ...
        centroids = [
            doc.to_dict()["centroid"] for doc in docs1_sorted
        ]  # 이전의 중심점들 정보를 받아오기

        kmeans = KMeans(n_clusters=len(centroids), init=centroids, random_state=0).fit(
            Datas
        )
...
def create_node_map(
    ...
    collection_ref = firestore_client.collection(TILES_COLLECTION)
    for doc in (
        collection_ref.document(first_tile).collection(NODES_COLLECTION).stream()
    ):
        node = Node(
            int(doc.id),
            doc.to_dict(),
...
@https_fn.on_call(
    timeout_sec=600, memory=options.MemoryOption.GB_4, region="asia-northeast3"
)
def request_route(req: https_fn.CallableRequest) -> RequestRouteReturn:
    ...
    try:
        ...
        open_tiles = set()
        getter_processes, putter_process = create_node_map(
            ...
            routes.append(
                {
                    ...
                    "full_distance": start_dist
                    + results[i]["full_distance"]
                    + end_dist,
                }
            )
